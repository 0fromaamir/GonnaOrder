...
export class CustomValidators {
    ...
    static stockemail(control: AbstractControl): ValidationErrors | null {
        if (isEmptyInputValue(control.value)) {
            return null;  // don't validate empty values to allow optional controls
        }
        return EMAIL_REGEXP.test(control.value.trim()) ? null : { stockemail: true };
    }
    ...
    static url(control: AbstractControl): ValidationErrors | null {	
        ...
        return URL_REGEXP.test(control.value.trim()) ? null : { "url": true };	
    }
    
    static maxLength20DigitsValidator(): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
          if (control.value === null || control.value === '') {
            return null;
          }
      
          const valueStr: string = control.value.toString().trim();
          if (!/^\d+$/.test(valueStr)) {
