...
import { getSelectedStore as getSelectedStorePublic } from '../../public/store/+state/stores.selectors';
...
export class PriceInputComponent implements OnInit, OnDestroy {
  ...
  selectedStorePublic$: Observable<ClientStore>;
  selectedStandalonePayment$: Observable<ClientStore>;
  priceSeparator: any;
  ...
  preparePriceDisplayFormat() {
    ...
    this.selectedStorePublic$ = this.store.pipe(select(getSelectedStorePublic));
    this.selectedStorePublic$
