...
import { Observable, Subject, combineLatest } from 'rxjs/index';
import { getMenuViewingStatistics, getOrderItemsStatisticsList, getSelectedStore, getSelectedStoreLocale } from '../+state/stores.selectors';
import { filter, takeUntil } from 'rxjs/operators';
import { StoreStatistics, ClientStore } from '../stores';
import { getStoreStatistics } from '../+state/stores.selectors';
import { DownloadOrderItemsXls, LoadMenuViewingStatistics, LoadOrderItemsStatisticsPage, LoadStoreStatistics } from '../+state/stores.actions';
...
export class StoreStatisticsComponent implements OnInit, OnDestroy {
  ...
  menuViewingStats: StoreStatistics[];
  total = 0;
  page: number;
  size: number;
  catalogId: any;
  ...
  ngOnInit() {
    ...
    combineLatest([
      ...
      this.store.select(getMenuViewingStatistics)
    ])
      .pipe(takeUntil(this.unsubscribe$), filter(s => !!s))
      .subscribe(([s, menuViewingStats]) => {
        console.log(menuViewingStats, "menuViewingStats")
        this.statistics = s;
        this.menuViewingStats = menuViewingStats;
        this.total = this.statistics.reduce((a, b) => +a + +b.total, 0);
        this.totalSubmitted = this.statistics.reduce((a, b) => +a + +b.totalSubmitted, 0);
        this.totalViewed = this.menuViewingStats.reduce((a, b) => +a + +b.totalViewings, 0);
  ...
  onTypeChange(type = null) {
    ...
    this.lineChartData = [];
    if (this.type.value !== this.statisticsTypes[1]) {
      ...
      if (this.periodicTerm === 'DAILY' && this.menuViewingStats.filter(a => !a.day).length === 0) {
        this.lineChartLabels = this.menuViewingStats.map(a => a.day.toString());
      }
      else {
        this.lineChartLabels = this.menuViewingStats.map(a => this.months[a.month - 1] + '-' + a.year);
      }
    }
    ...
    if (this.type.value === this.statisticsTypes[0]) {
      ...
      this.createMenuViewingChart(this.menuViewingStats);
      this.activeBtn = 1;
  ...
  loadStoreStatistics(duration) {
    ...
    this.store.dispatch(new LoadMenuViewingStatistics(
      duration, from.format('YYYY-MM-DD HH:mm:ss'),
      to.format('YYYY-MM-DD HH:mm:ss'),
      this.periodicTerm
    ))
