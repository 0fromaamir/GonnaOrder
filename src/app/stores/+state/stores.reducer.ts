...
export enum OrderingSetting {
  HIDDEN = 'HIDDEN',
  OPTIONAL = 'OPTIONAL',
  MANDATORY = 'MANDATORY'
}

export enum enableStandalonePaymentSetting {
  DISABLED = 'DISABLED',
  ENABLE_OPEN_TAP = 'ENABLE_OPEN_TAP'
}
export enum resrvationAllowCustomerSetting {
  DISABLED = 'DISABLED',
  ENABLE = 'ENABLED'
}

export enum standalonePaymentSourceSetting {
  GONNAORDER = 'GONNAORDER',
  POWERSOFT = 'POWERSOFT'
}
...
export interface StoreStatisticList {
  ...
  menuViewingStatistics: StoreStatistics[];
}
...
export function notificationSubscriptions(state: NotificationSubscriptions = storesInitialState['notificationSubscriptions'], action: StoresAction): NotificationSubscriptions {
  switch (action.type) {
    case StoresActionType.UpdateNotificationSubscriptions:
      console.log('UpdateNotificationSubscriptions: ', action.subscriptions);
  ...
  switch (action.type) {
    ...
    case StoresActionType.LoadStore:
      return {
        ...
        autoprintSupported: state.autoprintSupported,
      };
    ...
    case StorePaymentMethodsActionType.DisconnectTrustPaymentsFailed:
      return { ...state, status: 'FAILED' };
    case StorePaymentMethodsActionType.ConnectVivaPaymentsSuccess: {
      return { ...state, store: { ...state.store, settings: action.settings } };
    }
    case StorePaymentMethodsActionType.ConnectVivaPaymentsFailed:
      return { ...state, status: 'FAILED' };
    case StorePaymentMethodsActionType.DisconnectVivaPaymentsSuccess: {
      const settings = { ...state.store.settings };
      settings.PAYMENT_VIVA_PAYMENTS_CREDIT_CARD_ENABLED = false;
      settings.VIVA_MERCHANT_ID = '';
      return { ...state, store: { ...state.store, settings } };
    }
    case StorePaymentMethodsActionType.DisconnectVivaPaymentsFailed:
      return { ...state, status: 'FAILED' };
    case StorePaymentMethodsActionType.ConnectPayablSuccess: {
      return { ...state, store: { ...state.store, settings: action.settings } };
    }
    case StorePaymentMethodsActionType.ConnectPayablFailed:
      return { ...state, status: 'FAILED' };
    case StorePaymentMethodsActionType.DisConnectPayablSuccess: {
      const settings = { ...state.store.settings };
      settings.PAYMENT_PAYABL_CREDIT_CARD_ENABLED = false;
      settings.PAYABL_MERCHANT_ID = '';
      settings.PAYABL_MERCHANT_SECRET = '';
      return { ...state, store: { ...state.store, settings } };
    }
    case StorePaymentMethodsActionType.DisConnectPayablFailed:
      return { ...state, status: 'FAILED' };
    case StoresActionType.UpdateStoreRelation:
      return { ...state, status: 'LOADING' };
    case StoresActionType.UpdateStoreRelationSuccess: {
      return {
        ...state,
        store: {
          ...state.store,
          relation: {
            childStores: state.store.relation && state.store.relation.childStores ? state.store.relation.childStores : [],
            parentStore: {
              isIndependent: state.store.relation &&
                state.store.relation.parentStore && state.store.relation.parentStore.isIndependent
                ? state.store.relation.parentStore.isIndependent : false,
              storeAlias: action.parentStoreRelations.storeAlias,
              storeId: action.parentStoreRelations.storeId,
              storeName: action.parentStoreRelations.storeName
            },
            siblingStores: state.store.relation && state.store.relation.siblingStores ? state.store.relation.siblingStores : [],
          },
          isIndependent: action.parentStoreRelations.isIndependent
        },
        status: 'LOADED'
      };
    }
    case StoresActionType.UpdateStoreRelationFailed:
      return { ...state, status: 'FAILED' };
    case StoresActionType.DeleteStoreParent:
      return { ...state, status: 'LOADING' };
    case StoresActionType.DeleteStoreParentSuccess: {
      return {
        ...state,
        store: {
          ...state.store,
    ...
    case StoresActionType.DeleteStoreParentFailed:
      return { ...state, status: 'FAILED' };
  }
  return state;
}
...
export function selectedStoreImages(state: StoreImages = storesInitialState.selectedStoreImages, action: StoresAction): StoreImages {
  ...
  switch (action.type) {
    case StoresActionType.LoadStoreSuccess:
      return {
        imageUrl: action.store.settings.IMAGE, logoUrl: action.store.settings.LOGO,
        backgroundImageUrl: action.store.settings.IMAGE_BACKGROUND
      };
    case StoresActionType.UploadStoreImageSuccess:
      return action.backgroundImage ? { ...state, backgroundImageUrl: action.imageUrl } :
    ...
    case StoresActionType.RemoveStoreImageSuccess:
      return action.backgroundImage ? { ...state, imageUrl: null } : { ...state, backgroundImageUrl: null };
...
export function statisticsList(state: StoreStatisticList = storesInitialState.statisticsList, action: StoresAction): StoreStatisticList {
  ...
  switch (action.type) {
    ...
    case StoresActionType.LoadMenuViewingStatisticsSuccess:
      return { ...state, menuViewingStatistics: action.statistics };
  }
...
export interface CustomEndpointURL {
  customEndpointURL: string;
}

export const customEndpointInitialState: CustomEndpoint = {
  customEndpoint: {
    customEndpointURL: ''
  }
};
...
// tslint:disable-next-line
export function customEndpoint(state: CustomEndpointURL = customEndpointInitialState['customEndpoint'], action: StoresAction): CustomEndpointURL {
  switch (action.type) {
    case StoresActionType.CustomEndPointValidateSuccess:
      return { ...state, customEndpointURL: action.endpoint };
    case StoresActionType.CustomEndPointValidateFailed:
    default:
      return { ...state, customEndpointURL: '' };
  }
}

const reducerCustomEndpoint: (state: any, action: any) => CustomEndpoint = combineReducers({
  customEndpoint
});

export function customEndpointReducer(state: CustomEndpoint = customEndpointInitialState, action: StoresAction): CustomEndpoint {
  return reducerCustomEndpoint(state, action);
}
