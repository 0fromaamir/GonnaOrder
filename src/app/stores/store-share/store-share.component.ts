...
export class StoreShareComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.storesService.getUserInterfaceLanguages().pipe(
      ...
      for (let [index, browserLang] of preferredBrowserLanguages.entries()) {
        ...
        if (hasLocale) {
          this.browserDefaultLocale = browserLang;
          break;
        } else if (index === preferredBrowserLanguages.length - 1) {
          this.browserDefaultLocale = 'en';
        }
      }
    ...
    this.store$.pipe(
      ...
      .subscribe(store => {
        ...
        this.host = this.whitelabelService.getHost();
  ...
  copyEmbedCode() {
    navigator.clipboard.writeText(this.embedCode.toString());
  }
