...
import { LoadMenuViewingStatistics, LoadOrderItemsStatisticsPage, LoadStoreStatistics } from './+state/stores.actions';
...
export class StoreStatisticsGuard {
  ...
  canActivate(route: ActivatedRouteSnapshot) {
    ...
    return this.store.pipe(
      ...
      tap(_ => this.store.dispatch(new LoadMenuViewingStatistics(6, moment().subtract(6, 'months').startOf('month').format('YYYY-MM-DD'),
        moment().endOf('month').format('YYYY-MM-DD'), periodicTerm))),
