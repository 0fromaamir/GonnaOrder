...
import { AssociateCategoriesView, Category, DeleteDialogData, SaveOfferPositionView } from '../store-catalog/stores-catalog';
...
import { CategoryState, OfferState } from '../store-catalog/+state/stores-catalog.reducer';
import { LoadCategory, SaveOfferPosition } from '../store-catalog/+state/stores-catalog.actions';
...
export class StoreOrderingRulesComponent implements OnInit, OnDestroy {
  ...
  savePosition(direction, index) {
    ...
    if (direction === 'DOWN') {
      moveItemInArray(this.hasOffers, index, index + 1);
    } else {
      moveItemInArray(this.hasOffers, index - 1, index);
    }
    let i = 1;
    let offerPositionRequest: SaveOfferPositionView;
    offerPositionRequest = { offers: [] };
    const offerRequestList = [];
    this.hasOffers.forEach(item => {
      item.position = i;
      i++;
      offerRequestList.push({ offerId: item.offerId, position: item.position });
    });
    offerPositionRequest.offers = offerRequestList;
    this.offer.dispatch(new SaveOfferPosition(offerPositionRequest, this.storeId.toString(), this.catalogId.toString()));
  }
