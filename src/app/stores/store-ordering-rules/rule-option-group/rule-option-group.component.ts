...
export class RuleOptionGroupComponent implements OnInit, OnDestroy {
  ...
  onSaveRuleGroup() {
    ...
    this.store.dispatch(new SaveCategory(this.saveCategoryRequest, categoryId, this.storeId, this.catalogId, null,
      this.ruleId, null, null, null));
  }
  deleteCategory() {
    ...
    dialogRef.afterClosed().subscribe(action => {
      if (action !== undefined) {
        if (action === 'DELETE') {
          this.store.dispatch(new DeleteCategory(this.storeId, this.catalogId, this.ruleOptionGroupId, null, 0,
            null, null, null, null, null, null, this.ruleId));
        }
      }
    });
  }
