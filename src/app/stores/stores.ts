...
export interface ClientStoreRequest {
  ...
  subscription?: StoreSubscription;
  crmCompanyId?: string;
  firstLoginVoucherId?: number | null;
}
...
export interface Order {
  ...
  formattedServiceChargeAmount?: string;
  comment?: string;
  ...
  paymentStatus?: 'NO_PAYMENT' | 'IN_PROGRESS' | 'SUCCESSFULLY_COMPLETED' | 'FAILED';
  currency?: string;
  orderToken?: string;
  createdAt?: string;
  wishTime?: string;
  ...
  tapId?: number;
  external_address_delivery_id?: number;
...
export interface Relation {
  childStores: StoreRelations[];
  parentStore: StoreRelations;
  siblingStores: StoreRelations[];
}

export interface StoreRelationRequest {
  isIndependent: boolean;
  parentStoreId: number;
}

export interface CustomEndPointValidationRequest {
  endpoint: string;
}

export interface PresetOrderData {
  comment?: string;
  customerEmail?: string;
  customerName?: string;
  customerPhoneNumber?: string;
...
export enum StoreSaveMode {
  QRSCAN = "QRSCAN",
  URL = "URL",
  DEEPLINK = "DEEPLINK",
}

export interface StoreRecentsInfo {
  storeId: number;
  storeAlias: string;
  storeName: string;
  storeUrl: string;
  saveMode: StoreSaveMode;
  storeLogoUrl: string;
  lastOpen: string;
}
