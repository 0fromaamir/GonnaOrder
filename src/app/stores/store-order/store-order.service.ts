...
export class StoreOrderService {
  ...
  public listWithSortFilterParams(
    ...
    if (tabSortFilterParams.stationId) {
      params = {
        ...params,
        stationId: tabSortFilterParams.stationId
      }
    }
  ...
  public get(storeId: number, orderUuid: string, stationId?: number): Observable<Order> {
    if (stationId) {
      const params = {
        stationId: stationId
      }
      return this.http.get<Order>(`/api/v1/stores/${storeId}/orders/${orderUuid}`, { params });
    }
  ...
  downloadOrderPdf(storeId: number, orderId: string, stationId?: number): Observable<OrderPdfResponse> {
    const params = {
      stationId: stationId
    }
