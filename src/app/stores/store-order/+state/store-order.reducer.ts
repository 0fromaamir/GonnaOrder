...
export function list( state: StoreOrderList = storeOrdersInitialState.list, action: StoreOrderAction): StoreOrderList {
    switch (action.type) {
      ...
      case StoreOrderActionType.UpdateOrderStatusSuccess:
        const order = [...state.data].find(o => o.uuid === action.orderUuid);
        let data = [...state.data];
        if(order && !["SUBMITTED", "RECEIVED"].includes(order.status)){
          data = [...state.data].filter(o => o.uuid !== action.orderUuid);
        }
        return { ...state, data };
      default:
        return state;
