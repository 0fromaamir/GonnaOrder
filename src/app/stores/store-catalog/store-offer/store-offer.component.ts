...
export class StoreOfferComponent implements OnInit, OnDestroy {
  ...
  characterCountLong: number = 0; // Add characterCountLong variable
  characterCountPrice: number = 0; // Add characterCountPrice variable
  ...
  ngOnInit() {
    ...
    this.helpUrl = 'countAgainstSlotCount';
    ...
    this.offerForm = this.fb.group({
      ...
      priceDescription: [null, Validators.compose([Validators.minLength(2), Validators.maxLength(200)])],
    ...
    this.subscription = this.actions$.pipe(
      ...
      // console.log("save offer failed");
    });

    this.offerForm.get('shortDescription').valueChanges
      .pipe(takeUntil(this.destroy$))
      .subscribe(value => {
        this.characterCountShort = value ? value.length : 0; // Update character count
      });

      this.offerForm.get('longDescription').valueChanges
      .pipe(takeUntil(this.destroy$))
      .subscribe(value => {
        this.characterCountLong = value ? value.length : 0; // Update character count
      });

      this.offerForm.get('priceDescription').valueChanges
      .pipe(takeUntil(this.destroy$))
      .subscribe(value => {
      ...
      if (!this.isStoreOperator) {
        this.offer.dispatch(
          new SaveOffer(
            ...
            null, method, null, null, null, null, null, null, null, null, this.offerImage
          )
        );
  ...
  loadOfferVariantPage(offerId) {
    const qParam = {
      mainOfferId: this.offerId,
      mainOfferName: this.offerName,
    };
  ...
  deleteOffer() {
    ...
    dialogRef.afterClosed().subscribe(action => {
      if (action === 'DELETE') {
        this.offer.dispatch(new DeleteOffer(this.storeId, this.catalogId, this.offerId, null, null,
  ...
  isMaxLengthReached1(): boolean {
    return this.characterCountLong>= 1000; 
  }

  isMaxLengthReached2(): boolean {
  ...
  onLongDescriptionChange(value: string) {
    this.characterCountLong= value.length; 
  }
