...
import { Catalog, CatalogState } from '../+state/stores-catalog.reducer';
import {
  VerifyCatalogData,
  UploadCatalogData,
  VerifyCatalogImage,
...
export class CatalogUploadComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.isCategoryEmpty = !(this.importMode === 'EXCEL' || this.importMode === 'UPDATE_EXCEL' || this.importMode === 'UPDATE_EXCEL_OPTION_GROUP' || this.importMode === 'EXCEL_OPTION_GROUP');
    if(this.activeRoute.snapshot.queryParams.sourcePage === 'option-group'){
   ...
   setHelperPage(importMode: string): void {
    ...
    } else if(importMode === 'UPDATE_EXCEL_OPTION_GROUP' || importMode === 'EXCEL_OPTION_GROUP'){
      this.catalogUploadHelpPage = helpPage.catalogOptionsByExcel;
    }
  }
  ...
  verifyCatalog(fileInput: any) {
    if (this.importMode !== 'TRANSLATE') {
      ...
      this.file = file;
      this.catalog.dispatch(new VerifyCatalogData(file, this.storeId, this.catalogId, this.isOptionGroup));
  ...
  acceptAndUpload(event: any) {
    ...
    this.catalog.dispatch(new UploadCatalogData(this.catalogData, this.storeId, this.catalogId, this.isOptionGroup));
  }
  ...
  downloadToUpdateXls() {
    if(this.isOptionGroup){
      this.store.dispatch(new DownloadToUpdateCatalogXls(this.storeId, this.catalogId, 'option-group'));
    }else {
