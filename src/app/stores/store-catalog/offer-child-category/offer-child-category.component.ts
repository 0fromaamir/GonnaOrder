...
export class OfferChildCategoryComponent implements OnInit, OnChanges {
  ...
  ngOnInit() {
    ...
    this.offerView = this.route.snapshot.queryParams.offerView;
  ...
  setAssociateMode(event: MatOptionSelectionChange): void {
    if (event.isUserInput) {
      this.isAssociate = event.source.selected;
    }
  }
  ...
  loadChildCategoryPage(categoryId) {
    const qParam = {
      ...
      mainOfferId: this.mainOfferId ? this.mainOfferId : this.offerId,
      mainOfferName: this.mainOfferName ? this.mainOfferName : this.offerName,
  ...
  loadChildOfferPage(offerId, childCategoryId) {
    ...
    this.router.navigate(
      [ `/manager/stores/${this.storeId}/catalog/${this.catalogId}/childOffer`, offerId ],
      { queryParams: qParam }
    );
  }
