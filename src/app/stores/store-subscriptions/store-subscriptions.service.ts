import { StripeCheckoutSession, StorePurchase, StorePurchaceUpdateRequest, PurchaseInvoiceResponse, StorePurchaseStatus, StoreSubscription } from './subscriptions';
...
export class StoreSubscriptionsService {
  ...
  checkoutStripe(storeId: number, purchaseId: number, sharePaymentLink: boolean = false): Observable<StripeCheckoutSession> {
    return this.http.post<StripeCheckoutSession>(`/api/v1/subscriptions/checkout/stripe`, { storeId, purchaseId, sharePaymentLink});
  }
  ...
  updatePrice(storeId: number, body: any) {
    return this.http.patch(`/api/v1/stores/${storeId}`, body);
  }
