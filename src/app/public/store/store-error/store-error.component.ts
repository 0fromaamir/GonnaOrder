...
export class StoreErrorComponent implements OnInit, OnDestroy {
  ...
  OnGoToDashboard(event) {
    ...
    if (this.selectedStore && this.selectedStore.id && this.cartState && this.cartState.uuid) {
      this.store.dispatch(new CheckExistingOrder(this.selectedStore.id, this.cartState.uuid, 'CHECKEXISTING', null, null, {
        src: 'StoreErrorComponent',
  ...
  showReloadButton() {
    return this.errorMessage != 'public.checkout.errors.paymentFailed';
  }
}
