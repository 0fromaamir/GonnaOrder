...
@Component({
  selector: 'app-store-checkout-promotion',
  templateUrl: './store-checkout-promotion.component.html',
  styleUrls: ['./store-checkout-promotion.component.scss']
})
export class StoreCheckoutPromotionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.store.dispatch(new AddCheckoutState('storeRuleFormValid', true));
    ...
    , this.store.select(getCurrentCartState)
      ...
      .subscribe(state => {
        if (state) {
          ...
          this.checkoutService.setApplicableStoreRulesToInit();
  ...
  isApplicableStoreRule(rule) {
    ...
    rule.conditions.forEach(condition => {
      switch (condition.type) {
        case 'ORDER_AMOUNT_RANGE':
          if (this.checkoutService.getCartTotal(true) < condition.data.min
            || this.checkoutService.getCartTotal(true) > condition.data.max) {
    ...
    if (!allActionsHaveOffers || !allConditionsAreTrue) {
      return false;
    }
    return true;
  }
  ...
  radioButtonValidationRules() {
    ...
    this.storeRules.forEach((rule,  ruleIndex) => {
      ...
      if (this.isApplicableStoreRule(rule)) {
        this.checkoutService.setApplicableStoreRule(rule);
        ...
        rule.actions.forEach((action, actionIndex) => {
          if (action.type === 'ADD_FREE_CATEGORY') {
            this.offerVariants[ruleIndex][actionIndex] = [];
            this.radioCategoriesFgArr[ruleIndex][actionIndex] = [];
            if (!action.data.offers) {
              return;
            }
            // parse all offers for current RULE category:
            if (action.data.min === 0 && action.data.max === 1) {
              this.childCheckboxAsRadioOfferItemsList[action.data.categoryId] = [];
            }
            if (action.data.min === 1 && action.data.max === 1 && action.data.offers) {
              let defaultValue = '';
              if (action.data.offers) {
              ...
              this.store.dispatch(new AddCheckoutState('storeRuleFormValid', !!defaultValue));
            } else {
              if (action.data.offers) {
                action.data.offers.forEach(offerItem => {
                  if (action.data.min === 0 && action.data.max === 1) {
                    this.childCheckboxAsRadioOfferItemsList[action.data.categoryId].push(offerItem.offerId);
                  }
                  if (childOrderItems.includes(offerItem.offerId) || this.checkoutService.ifCartOfferRule(offerItem.offerId)) {
                    this.childSelectedCheckboxOfferItems.push(offerItem);
                  }
                  radioGroups['var' + offerItem.offerId] = [
                    (
                      (childOrderItems.includes(offerItem.offerId)
                      || this.checkoutService.ifCartOfferRule(offerItem.offerId)) ? offerItem.offerId : ''
                    ),
                    []
                  ];
                });
  ...
  OnSelectOfferVariant(ruleIndex, offerItem) {
    ...
    if (!this.radioCategoriesFg.valid) {
      this.store.dispatch(new AddCheckoutState('storeRuleFormValid', false));
      // return;
    } else {
      this.store.dispatch(new AddCheckoutState('storeRuleFormValid', true));
    }
    ...
    if (childItemRequests.length > 0) {
      ...
      */
      // this.store.dispatch(new AddCheckoutState('storeRuleOffers', childItemRequests));
