...
public class BluetoothSerial extends CordovaPlugin {
    ...
    public static Queue<PrinterDetails> printQueue = new LinkedList<>();
    private static final UUID BLUETOOTH_SPP = UUID.fromString("00001101-0000-1000-8000-00805F9B34FB");
    private enum Connected { False, True };
    ...
    public void connectPrinter() {
        cordova.getThreadPool().execute(() -> {
            if (printQueue != null && !printQueue.isEmpty()) {
                PrinterDetails printerDetails = printQueue.peek();
                connectToPrinter(printerDetails.device, printerDetails.data, printerDetails.connectCallback);
            }
        });
    }

    synchronized private void connectToPrinter(BluetoothDevice device, byte[] data, CallbackContext connectCallback) {
        try {
            if (socket == null) {
            ...
            socket.getOutputStream().write(Command.DLE_eot);
            socket.getInputStream().read(new byte[1]);
            notifyConnectionSuccess(connectCallback);
        } catch (Exception e) {
            try {
                socket.close();
                notifyConnectionLost(e.getMessage(), connectCallback);
            } catch (Exception ex) {
                ex.printStackTrace();
                notifyConnectionLost(ex.getMessage(), connectCallback);
            }
            socket = null;
        } finally {
    ...
    synchronized public void printByte(byte[] data) {
        try {
            for (int i = 5; i > 0 && (socket == null || !socket.isConnected()); i--) {
                Thread.sleep(1000);
            }
            socket.getOutputStream().write(data);
        } catch (Exception e) {
    ...
    synchronized private void printString(String msg) {
        if (msg.length() > 0) {
            try {
    ...
    private void connect(CordovaArgs args, boolean secure, CallbackContext callbackContext) throws JSONException {
        ...
        if (device != null) {
            ...
            printQueue.add(new PrinterDetails(device, data, connectCallback));
            if (printQueue.size() == 1) connectPrinter();
            buffer.setLength(0);
    ...
    void notifyConnectionLost(String error) {
    ...
    void notifyConnectionLost(String error, CallbackContext connectCallback) {
        connected = Connected.False;
        if (connectCallback != null) {
            connectCallback.error(error);
            connectCallback = null;
    ...
    void notifyConnectionSuccess(CallbackContext connectCallback) {
        connected = Connected.True;
        if (connectCallback != null) {
            PluginResult result = new PluginResult(PluginResult.Status.OK);
            result.setKeepCallback(true);
...
class PrinterDetails {
    ...
    byte[] data;
    CallbackContext connectCallback;

    PrinterDetails(BluetoothDevice device, byte[] data, CallbackContext connectCallback) {
        this.device = device;
        this.data = data;
